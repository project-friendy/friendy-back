...
public interface SpringDocMemberController {
    ...
    @ApiErrorResponse(status = HttpStatus.BAD_REQUEST, instance = "/signup", errorCases = {
            ...
            @ErrorCase(description = "지원되지 않는 파일 확장자", exampleMessage = "지원되지 않는 파일 확장자입니다."),
            @ErrorCase(description = "파일 크기가 허용된 범위 넘음", exampleMessage = "파일 크기가 허용된 범위를 초과했습니다.")
    ...
    ResponseEntity<Void> signUp(
        @RequestPart(value = "request") MemberSignUpRequest request,
